...
/*
a) select count (*) as quantidade from pizza
where comanda = 235;

b) select count (*) as quantidade from pizza
where tamanho = 'G' | 'F' and comanda = 235;

c)select count(*) as quantidade from comanda
where date(data) > date('now', '- 7 days' ,'localtime')and pago = 0;

d) select count (*) as quantidade from saboringrediente
where sabor = 123; 

e) select count (*) as quantidade from saboringrediente
where ingrediente = 234;

f)select (count(ingrediente)/count(distinct sabor)) as media from saboringrediente
group by sabor;

g) select sabor, count(*) as quantidade from saboringrediente
group by sabor
having count(*) > 8;

h) select sabor, count(*) as quantidade from saboringrediente
where sabor = 5 | 6
group by sabor
having count(*) > 8;

i) select date(data), count(*) as quantidade from comanda
where date(data) > date('now', '-15 days', 'localtime');

j)select date(data), count(*) as quantidade from comanda
where date(data) > date('now', '- 15 days', 'localtime')
group by data
having count(*)> 10;

k)select count(*) as quantidade, 
case strftime('%w', 'now', 'localtime')
	...
	from comanda
	where date('now', '-12 months', 'localtime')
	group by dia
order by quantidade;

l)select count(*) as quantidade, 
case strftime('%w', 'now', 'localtime')
        when '0' then 'domingo'
        when '1' then 'segunda'
        when '2' then 'terca'
	...
	from comanda
	where date('now', '-1 month', 'localtime')
	group by dia
order by quantidade;

m)select count(*) as quantidade, 
case strftime('%w', 'now', 'localtime')
        when '0' then 'domingo'
        when '1' then 'segunda'
        when '2' then 'terca'
...
n) Não da pra fazer pois não temos como ver os sabores mais pedidos

o) Não da pra fazer pois não tem como mostrar o valor, so vai retornar true or false 
*/
